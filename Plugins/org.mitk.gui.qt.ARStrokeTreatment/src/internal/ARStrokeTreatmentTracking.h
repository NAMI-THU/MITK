...
public:
  ...
  ARStrokeTreatmentTracking();
  ~ARStrokeTreatmentTracking() override;
...
protected:
  ...
  mitk::TrackingDeviceData m_TrackingDeviceData; ///> stores the tracking device data as long as this is not handled by
                                                 ///the tracking device configuration widget
  mitk::NavigationDataObjectVisualizationFilter::Pointer
    m_ToolVisualizationFilter; ///> holds the tool visualization filter (second filter of the IGT pipeline)
...
#endif // ARStrokeTreatmentTracking_h
